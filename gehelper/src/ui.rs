...
use gehelper_lib::archive;
use gehelper_lib::config::{LutrisConfig, SteamConfig};
use gehelper_lib::download::response::DownloadedAssets;
use gehelper_lib::download::{DownloadRequest, GeDownload};
use gehelper_lib::error::{GithubError, LutrisConfigError, SteamConfigError};
...
impl<'a> TerminalWriter<'a> {
    ...
    fn get_app_name(&self, kind: &TagKind) -> String {
        match kind {
            TagKind::Proton => "Steam",
            TagKind::Wine { .. } => "Lutris",
        }
        .parse()
        .unwrap()
    ...
    pub fn list(&self, stdout: &mut impl Write, args: ListArgs, config_paths: AppConfigPaths) -> anyhow::Result<()> {
        ...
        if !managed_versions.is_empty() {
            ...
            for (kind, group) in &grouped_versions {
                ...
                let versions: Vec<ManagedVersion> = group.collect();
    ...
    pub fn add(&self, stdout: &mut impl Write, args: AddArgs) -> anyhow::Result<()> {
        ...
        let assets = match self.ge_downloader.download_release(request) {
            Ok(assets) => assets,
            Err(err) => {
    ...
    pub fn remove(
        ...
        match &version.kind() {
            TagKind::Proton => {
                let config = SteamConfig::create_copy(&config_paths.steam);
                self.check_if_version_in_use(&version, config)?;
            }
            TagKind::Wine { .. } => {
                let config = LutrisConfig::create_copy(&config_paths.lutris);
                self.check_if_version_in_use(&version, config)?;
            }
        }
    ...
    fn check_if_version_in_use<T, E>(&self, version: &ManagedVersion, app_config: Result<T, E>) -> anyhow::Result<()>
    where
        T: AppConfig,
        E: Into<anyhow::Error> + AppConfigError,
    {
        if let Ok(config) = app_config {
            if config.version_dir_name().eq(version.directory_name()) {
                bail!(format!(
                    "Version is in use: Select a different version in {} to make removal possible",
                    config.kind()
                ));
            }
            Ok(())
            ...
            let kind = err.kind();
            Err(anyhow!(err).context(format!("Failed to get a copy of the {} config", kind)))
        }
    }
    ...
    fn do_apply_to_app_config(&self, stdout: &mut impl Write, version: &ManagedVersion) -> anyhow::Result<()> {
        let app_name = self.get_app_name(version.kind());

        writeln!(stdout, "Modifying {} configuration to use {}", app_name, version).unwrap();
        ...
        writeln!(
            stdout,
            "Successfully modified config: {} should be restarted for the new settings to take effect.",
            app_name
        )
        .unwrap();
    ...
    pub fn forget(&self, stdout: &mut impl Write, args: ForgetArgs) -> anyhow::Result<()> {
        ...
        self.write_managed_versions(managed_versions)?;
        writeln!(stdout, "{} is now not managed by GEhelper", version).unwrap();
        Ok(())
    }
}
...
mod tests {
    ...
    use gehelper_lib::download::response::{DownloadedChecksum, DownloadedTar, GeRelease};
    use gehelper_lib::tag::Tag;
    use mockall::mock;
    ...
    fn forget_should_print_success_message() {
        ...
        let json_path = tmp_dir.join("gehelper/managed_versions.json");
        ...
        stdout.assert_line(0, "6.20-GE-1 (Proton) is now not managed by GEhelper");
    }
    ...
    fn list_newest_output() {
        ...
        let config_paths = AppConfigPaths::new("tests/resources/assets/config.vdf", "tests/resources/assets/wine.yml");
        let mut stdout = AssertLines::new();
    ...
    fn add_successful_output() {
        ...
        ge_downloader.expect_download_release().once().returning(|_| {
            Ok(DownloadedAssets {
                tag: "".to_string(),
                compressed_tar: DownloadedTar {
    ...
    fn add_with_checksum_comparison_successful_output() {
        ...
        let mut ge_downloader = MockDownloader::new();
        ge_downloader.expect_download_release().once().returning(|_| {
            let tar = fs::read("tests/resources/assets/Proton-6.20-GE-1.tar.gz").unwrap();
            let checksum = fs::read_to_string("tests/resources/assets/Proton-6.20-GE-1.sha512sum").unwrap();
    ...
    #[test]
    fn add_with_update_config_should_modify_app_config() {
        let tag_arg = TagArg::new(Some(Tag::from("6.20-GE-1")), TagKind::Proton);
        ...
        stdout.assert_line(0, "Performing checksum comparison: Checksums match");
        stdout.assert_line(1, "Modifying Steam configuration to use 6.20-GE-1 (Proton)");
        stdout.assert_line(
            2,
            "Successfully modified config: Steam should be restarted for the new settings to take \
        effect.",
        );
        stdout.assert_line(3, "Successfully added version");
    }
    ...
    fn remove_existing_version() {
        ...
        let config_path = PathBuf::from("tests/resources/assets/config.vdf");
    ...
    fn remove_version_used_by_app_config() {
        ...
        assert_eq!(
            err.to_string(),
            "Version is in use: Select a different version in Steam to make removal possible"
        );
        stdout.assert_empty();
    }
    ...
    fn apply_to_app_config_for_latest_version() {
        ...
        stdout.assert_line(
            1,
            "Successfully modified config: Steam should be restarted for the new settings to take effect.",
        );
    }
