...
impl<'a> FilesystemManager for FsMng<'a> {
    fn setup_version(&self, version: Version, compressed_tar: Box<dyn Read>) -> anyhow::Result<ManagedVersion> {
        ...
        let extracted_location = archive::extract_compressed_tar(version.kind(), compressed_tar, &dst_path)
...
mod tests {
    ...
    fn setup_proton_version() {
        ...
        let compressed_tar = BufReader::new(File::open("tests/resources/assets/Proton-6.20-GE-1.tar.gz").unwrap());
    ...
    fn setup_wine_version() {
        ...
        let compressed_tar = BufReader::new(File::open("tests/resources/assets/Wine-6.20-GE-1.tar.xz").unwrap());
        let version = Version::new(tag.clone(), kind.clone());
        let managed_version = fs_manager.setup_version(version, Box::new(compressed_tar)).unwrap();
    ...
    fn setup_wine_lol_version() {
        ...
        let compressed_tar = BufReader::new(File::open("tests/resources/assets/Wine-6.20-GE-1-LoL.tar.xz").unwrap());
        let version = Version::new(tag.clone(), kind.clone());
        let managed_version = fs_manager.setup_version(version, Box::new(compressed_tar)).unwrap();
    ...
    fn modify_config_with_proton_version() {
        ...
        fs::copy("tests/resources/assets/config.vdf", &cfg_file).unwrap();
    ...
    fn modify_config_with_wine_version() {
        ...
        let dir_name = "Wine-6.20-GE-1";
        fs::create_dir_all(&cfg_dir).unwrap();
        fs::copy("tests/resources/assets/wine.yml", &cfg_file).unwrap();
    ...
    fn copy_proton_settings() {
        ...
        let src_tar = File::open("tests/resources/assets/Proton-6.19-GE-1.tar.gz").unwrap();
        let dst = Version::new("6.20-GE-2", TagKind::Proton);
        let dst_tar = File::open("tests/resources/assets/Proton-6.20-GE-2.tar.gz").unwrap();
